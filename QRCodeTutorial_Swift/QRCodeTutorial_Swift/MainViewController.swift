...
//
//  Created by 박은아 on 2022/01/27.
//

import UIKit
import WebKit
import AVFoundation //사진에서 사용할 때 쓰는 프레임워크
import QRCodeReader //QR 코드 읽기
...
class MainViewController: UIViewController, QRCodeReaderViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        qrcodeBtn.addTarget(self, action: #selector(qrcodeReaderLunch), for: .touchUpInside)
        
    }


    //MARK: - fileprivate 메소드
    @objc fileprivate func qrcodeReaderLunch(){
        print("MainViewController - qrcodeReaderLunch() called")
        ...
        // Retrieve the QRCode content
          ...
          readerVC.completionBlock = { (result: QRCodeReaderResult?) in
            print(result)
              ...
              self.webView.load(URLRequest(url:scannedUrl!)) //!반드시 값이 있다 라는 설정
